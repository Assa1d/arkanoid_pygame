...
class GameScreen:
    ...
    def _draw(self) -> None:
        ui.draw_gradient_background(self.screen)
        ...
        # HUD bar panel across the top of the field
        hud_rect = pygame.Rect(cfg.FIELD_LEFT - 8, 4, cfg.FIELD_RIGHT - cfg.FIELD_LEFT + 16, 34)
        ui.draw_panel(self.screen, hud_rect, radius=8)
        score_text = self.hud_font.render(f"Score: {self.score}", True, cfg.GOLD)
        lives_text = self.hud_font.render(f"Lives: {self.lives}", True, cfg.ACCENT)
        self.screen.blit(score_text, (hud_rect.left + 10, 12))
        self.screen.blit(lives_text, lives_text.get_rect(topright=(hud_rect.right - 10, 12)))

        ui.draw_frame(self.screen)
